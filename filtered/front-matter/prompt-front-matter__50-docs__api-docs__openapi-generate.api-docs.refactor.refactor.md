...
- Command: `/openapi-generate {{1}} {{2}} {{3}}`
- Parameters:
  - `{{1}}`: Generates controllers, routers, validation, and error middleware into `apps/api`
  - `{{2}}`: Generates a typed SDK into `packages/sdk` with fetch wrapper and retry/backoff
  - `{{3}}`: Path to OpenAPI spec (e.g., `apis/auth/openapi.yaml`)
...
Trigger: /openapi-generate {{1}} {{2}} {{3}}

Purpose: Generate server stubs or typed clients from an OpenAPI spec.

**Steps:**

1. Validate `{{3}}`; fail with actionable errors.
2. For `{{1}}`, generate controllers, routers, validation, and error middleware into `apps/api`.
3. For `{{2}}`, generate a typed SDK into `packages/sdk` with fetch wrapper and retry/backoff.
...
**Examples:** `/openapi-generate {{1}} {{2}} {{3}}`.
