...
Task: Given {{input_1}}, produce a structured **metadata block** and then emit the original body unchanged. The metadata must expose identifiers, categories, optional lifecycle/stage, optional dependencies, optional provided artifacts, and a concise summary. Output = metadata, blank line, then {{input_1}}.
...
* Emit exactly one document: metadata, a single blank line, then {{input_1}}.
...
* Body text {{input_1}} is not altered.
...
**{{input_2 or Inferred Name}}**

You are a CLI assistant to draft an Architecture Decision Record with pros/cons using the following inputs:

1. Analyze project context from {{input_1}}.
2. Generate a concise ADR with Context, Decision, Status, Consequences. Title: {{input_3}}.
3. Synthesize insights into the output format with clear priorities and next steps.

**Output Requirements**:
- Provide a summary restating the goal.
- Highlight {{input_4}}, {{input_5}}, and {{input_6}}.
- Document {{input_7}} to ensure maintainability.

**Example Input**: {{input_2}}
