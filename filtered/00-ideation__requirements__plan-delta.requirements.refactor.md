...
2. Snapshot:
   ...
   2. Copy the current tasks file to `./artifacts/tasks-{0}.json` using: `cp -f <tasks.json> ./artifacts/tasks-{0}.json`.
3. Input collection:
...
4. Delta Doc generation:
   1. Create `./artifacts/delta-{1}.md` containing sections:
      - Objectives (new)
      - Constraints (new)
      - Impacts
      - Decisions
...
- Input →
  ...
  Output →
  - Updated `tasks.json` with new task `{2}` { title: "Add CSV export", dependencies: ["{3}"], source_doc: "delta-{1}", lineage: ["{3}"], supersedes: [] }.
  - `artifacts/delta-{1}.md` populated with objectives, constraints, impacts, decisions, evidence.
  - Readiness report lists `{2}` under READY if deps done.
